...
pkgs.testers.nixosTest {
  ...
  testScript = ''
    ...
    with subtest("enforce the global concurrency limit"):
    ...
    with subtest("enforce the per-chat concurrency limit"):
        inject("chat-a", 2001)
    ...
    with subtest("reject requests above the per-chat admission limit"):
        inject("rate-a", 3001)
